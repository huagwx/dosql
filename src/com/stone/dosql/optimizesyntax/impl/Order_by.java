...
public class Order_by {
	...
	public Order_by(Syntax groupbySyntax,List fromTableList) {
		
		this.orderbySyntax=groupbySyntax;
		this.fromTableList=fromTableList;	
		this.doAction(this.orderbySyntax);
	}
	...
	private void doAction(Syntax syntax){
		...
		if(syntax.getLeft().getType()==ST_TYPE.ST_VN&&syntax.getLeft().getName().equals(this.column_ref)){
			...
			//System.out.println("this.listIdent.size()=="+this.listIdent.size());
//			for(int i=0;i<this.listIdent.size();i++){
//				System.out.println((String)this.listIdent.get(i));
//			}
