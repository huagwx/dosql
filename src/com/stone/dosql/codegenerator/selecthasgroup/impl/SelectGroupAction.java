...
import com.stone.dosql.codegenerator.select.impl.SelColumnTemp;
import com.stone.dosql.codegenerator.select.impl.SelectTemp;
import com.stone.dosql.codegenerator.select.impl.SelectTempClone;
...
public class SelectGroupAction {
	...
	//private List groupbyColumnList;   //groupby的列
	//this.groupByTempList.
	//this.selectMiddleCodeList,this.resultSetWhere,this.resultSetTemp
	public SelectGroupAction(List selectMiddleCodeList,ResultSetInit resultSetWhere,ResultSetTemp resultSetTemp) {
	...
	private int recordId;  //当前记录id号 在groupby的时候map对应
	private void doEachRecord(){
		int recSize=this.resultSetWhere.getRecordList().size();
		for(int i=0;i<recSize;i++){
			if(this.resultSetWhere.getGroupbyArray()[i]){  //为true时候就不做 groupby后选择的记录是groupbyflag为false的
				continue;
			}
			this.recordId=i;
	...
	private double getMin(String t1,String t2){
		...
		int size=this.resultSetWhere.getRecordList().size();
			...
			if(col.getType().equals("DOUBLE")||col.getType().equals("INTEGER")){
				...
				String strT=(String)this.oneRecord.get(ct.getId());
				double dt;
				try {
					dt=Double.valueOf(strT);
				...
				for(int i=0;i<size;i++){
					if(this.resultSetWhere.getGroupbyArray()[i]){ //为true就要操作
						if(this.resultSetWhere.getGroupMapId()[i]==this.recordId){  //更现在取的记录id相同
							List listT=(List)this.resultSetWhere.getRecordList().get(i);
							String strTT=(String)listT.get(ct.getId());
							double dtt;
							try {
								dtt=Double.valueOf(strTT);
							} catch (NumberFormatException e) {
								// TODO Auto-generated catch block
								dtt=0;
							}
...
//				object=allSum/size;
			...
			if(col.getType().equals("DOUBLE")||col.getType().equals("INTEGER")){
				List comList=new ArrayList();  //保存前面存入的值 用于后面对比有没有相同的
				double allSum=0;
				int allNum=0;
				String strT=(String)this.oneRecord.get(ct.getId());
     			comList.add(strT);    //
				...
				for(int i=0;i<size;i++){
					if(this.resultSetWhere.getGroupbyArray()[i]){ //为true就要操作
						if(this.resultSetWhere.getGroupMapId()[i]==this.recordId){  //更现在取的记录id相同
							...
							boolean isContinue=true;
							for(int ci=0;ci<comSize;ci++){
								String comStr=(String)comList.get(ci);
								if(strTT.equals(comStr)){
									isContinue=false;
									break;
								}
							}
...
//				object=iTemp;
			...
			if(t1.equals("*")||t1.endsWith(".*")){
				List comList=new ArrayList();  //保存前面存入的值 用于后面对比有没有相同的
				int temp=1;
				comList.add(this.oneRecord);
				for(int i=0;i<size;i++){
					if(this.resultSetWhere.getGroupbyArray()[i]){ //为true就要操作
						if(this.resultSetWhere.getGroupMapId()[i]==this.recordId){  //更现在取的记录id相同
							List curRec=(List)this.resultSetWhere.getRecordList().get(i);
							int comSize=comList.size();
							int ci;
							for(ci=0;ci<comSize;ci++){
								List oneRec=(List)comList.get(ci);
								int recSize=oneRec.size();
								
								int rci;
								for(rci=0;rci<recSize;rci++){
									String str=(String)oneRec.get(rci);
									String curStr=(String)curRec.get(rci);
				...
				for(int i=0;i<size;i++){
					if(this.resultSetWhere.getGroupbyArray()[i]){ //为true就要操作
						if(this.resultSetWhere.getGroupMapId()[i]==this.recordId){  //更现在取的记录id相同
							...
							for(ci=0;ci<comSize;ci++){
								List oneRec=(List)comList.get(ci);
								String str1=(String)curRec.get(ct.getId());
								String str2=(String)oneRec.get(ct.getId());
								if(str1.equals(str2)){
									break;
								}
