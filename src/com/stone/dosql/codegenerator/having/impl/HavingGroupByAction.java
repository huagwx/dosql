...
public class HavingGroupByAction {
	//this.havingMiddleCodeList,this.resultSetTemp,this.resultSetTempList
	private List havingMiddleCodeList;
	private ResultSetTemp resultSetTemp;
	private List resultSetTempList;
	private List tempList=new ArrayList();
	public HavingGroupByAction(List havingMiddleCodeList,ResultSetTemp resultSetTemp,List resultSetTempList) {
		this.havingMiddleCodeList=havingMiddleCodeList;
	...
	private void doEachRecord(){
		int recSize=this.resultSetTemp.getResultSet().getRecordList().size();
		//设置resultset的记录标识为false  当有用的时候就设置为true 最后选择为true的记录
		this.resultSetTemp.getResultSet().setFlagArray(new boolean[recSize]);
		for(int i=0;i<recSize;i++){
			this.oneRecord=(List)this.resultSetTemp.getResultSet().getRecordList().get(i);
			boolean is=this.doAction();
			if(is){
				this.resultSetTemp.getResultSet().getFlagArray()[i]=true;
			}
		}
//		处理掉为false的记录
		int falgsize=this.resultSetTemp.getResultSet().getFlagArray().length;
		for(int i=0;i<falgsize;i++){
			
			if(this.resultSetTemp.getResultSet().getFlagArray()[i]==false){
				this.resultSetTemp.getResultSet().getRecordList().set(i, null);
			}
		}
		//删除为null的record
		this.deleteResultSetHasNull();
		//返回的结果的flag都设置为flag
		int leftsize=this.resultSetTemp.getResultSet().getRecordList().size();
		this.resultSetTemp.getResultSet().setFlagArray(new boolean[leftsize]);
	}
	//清楚记录中含有null的记录
	private void deleteResultSetHasNull(){
	...
	private boolean doAction(){
		boolean is=false;
		int size=this.havingMiddleCodeList.size(); //没有from和where
		for(int i=0;i<size;i++){
			List oneLine=(List)this.havingMiddleCodeList.get(i);
...
//        	while(isContinue){
//            	try {
//					String t1T=t1.substring(t1P,t1P+1);
//					String t2T=t2.substring(t2P, t2P+1);
//					
//					if(t2T.equals("_")){
//						if(t3.equals("-")){
//							t1P++;
...
//        	}
	...
	private ColumnTemp  getColumnTempByFullName(String columnFullName){
		...
		List list=this.resultSetTemp.getResultSet().getSelColumnsList();
		int size=list.size();
		for(int i=0;i<size;i++){
