...
public class HavingWhereDo {
//this.allMiddleCodeListOfOneQuery,resultSetWhere,this.resultSetTempList
	...
	//返回resultSet的结果
	// 返回的结果的flag都是false标识
	public HavingWhereDo(List allMiddleCodeListOfOneQuery,ResultSetInit resultSetWhere,List resultSetTempList,List recordTableList) {
...
//			this.doAllSet();
			...
			if(size==0){
				...
				this.havingAction=new WhereAction(this.havingMiddleCodeList,this.resultSetWhere,this.resultSetTempList);
		...
		private void doWhereConditionNotNull(){
			...
			for(int ri=0;ri<allRecordSize;ri++){
				...
				if(this.resultSetWhere.getFlagArray()[ri]==true){
					...
					this.resultSetWhere.getFlagArray()[ri]=this.havingAction.getBoolResult((List)this.resultSetWhere.getRecordList().get(ri));
